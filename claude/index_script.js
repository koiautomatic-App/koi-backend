/* ═══════════════════════════════════════════════════════════
   KOI-FACTURA · index.html — BLOQUE <script> CORREGIDO
   
   REEMPLAZA TODO EL CONTENIDO entre <script> y </script>
   (los dos bloques <script> al final del body).
   
   Correcciones aplicadas:
   1. CSS switches: usa .sw / .sw-track (coincide con el CSS existente)
   2. emitir(id) llama a POST /api/orders/:id/emitir real
   3. DOMContentLoaded corregido — sin referencias a #dashPeriod inexistente
   4. _recargarDashConPeriodo() pasa ?desde=&hasta= al server (filtro real)
   5. cargarConfigVista() carga factAuto/envioAuto reales del usuario
   6. guardarSwitch() guarda via PATCH /api/me/settings
   7. toggleClave() y actualizarEstadoARCA() unificados y sin duplicados
   ═══════════════════════════════════════════════════════════ */
...
const MOCK = {
  ...
  monoLimite:    2432364,
  monoMes:       "Período: Este mes",
  hoyFacturado:  48500,  hoyDelta: "ventas hoy",    hoyTipo: "up",
  pendientesCAE: 3,      pendDelta: "sin emitir",   pendTipo: "warn",
  mesFacturado:  875420, mesDelta: "con CAE",        mesTipo: "up",
  ...
  chartInst = new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        ...
        pointRadius: 0, pointHoverRadius: 5, pointHoverBackgroundColor: '#00e676', tension: 0.45,
      }],
    },
    options: {
      ...
      plugins: {
        ...
        tooltip: {
          backgroundColor: '#17172a', borderColor: 'rgba(0,230,118,0.2)', borderWidth: 1,
          ...
          padding: 10, callbacks: { label: ctx => ` ${ars(ctx.raw)}` },
        },
      },
      scales: {
...
function renderComps(lista) {
  ...
  if (!lista.length) {
    cont.innerHTML = `<div style="padding:30px;text-align:center;color:var(--text-3);font-size:12px">Sin ventas este período</div>`;
    return;
  }
  cont.innerHTML = lista.map((c, i) => {
    const emitido  = c.estado === 'cae-ok';
    const btnEmitir = emitido
      ? `<button class="act-btn act-done" disabled title="Emitido ✓">
           <svg width='13' height='13' viewBox='0 0 14 14' fill='none'>
             <path d='M2.5 7l3 3 6-6' stroke='currentColor' stroke-width='1.4' stroke-linecap='round' stroke-linejoin='round'/>
           </svg>
         </button>`
      : `<button class="act-btn" title="Emitir CAE" onclick="emitir('${c.id}')">
           <svg width='13' height='13' viewBox='0 0 14 14' fill='none'>
    ...
    <div class="comp-row" style="animation-delay:${i*55}ms">
      ...
      <div class="comp-actions">
        <button class="act-btn" title="Ver PDF" onclick="verPDF('${c.id}')">
          <svg width="13" height="13" viewBox="0 0 14 14" fill="none">
        ...
        ${btnEmitir}
        <button class="act-btn" title="Enviar mail" onclick="enviarMail('${c.id}')">
          <svg width="13" height="13" viewBox="0 0 14 14" fill="none">
...
/* ── ADAPTAR RESPUESTA DE /api/stats/dashboard ─────── */
function adaptarStats(raw) {
  ...
  // Agrupar ventas del período por día (usa orderDate si existe, sino createdAt)
  const ventasPorDia = {};
  (raw.ultimas || []).forEach(o => {
    const fecha = o.orderDate
  ...
  // Bandeja: últimas 20 órdenes del período
...
function _syncDashInputs() {
  ...
  const elD = document.getElementById('dashDesde');
  const elH = document.getElementById('dashHasta');
  if (elD) elD.value = toISO(_dashDesde);
  if (elH) elH.value = toISO(_dashHasta);
}
...
/** Recarga el dashboard pasando desde/hasta al server — el filtro lo hace MongoDB */
function _recargarDashConPeriodo() {
  ...
  api.get(url)
    .then(raw => {
      ...
      if (!data) return;
      // Actualizar etiqueta del período en la card mono
...
function mostrarVista(v) {
  ...
  const vistas = {
    dashboard:    { el: () => document.querySelector('.content'), nav: 'nav-dashboard' },
    comprobantes: { el: () => document.getElementById('vista-comprobantes'), nav: 'nav-comprobantes', fn: () => { if (!_rangoDesde) _iniciarPeriodo(); cargarTodosComprobantes(); } },
    negocio:      { el: () => document.getElementById('vista-negocio'),      nav: 'nav-negocio',      fn: cargarIntegraciones },
    arca:         { el: () => document.getElementById('vista-arca'),          nav: 'nav-arca',          fn: verificarEstadoArca },
    config:       { el: () => document.getElementById('vista-config'),        nav: 'nav-config',        fn: cargarConfigVista },
    estado:       { el: () => document.getElementById('vista-estado'),        nav: 'nav-estado',        fn: verificarSuscripcion },
  };
  ...
  const navEl = document.getElementById(vista.nav);
...
/** Carga el estado real de factAuto / envioAuto del server */
async function cargarConfigVista() {
  try {
    ...
    // Perfil
    const n2 = document.getElementById('cfgNombre2');
    const c2 = document.getElementById('cfgCuit2');
    const cat2 = document.getElementById('cfgCategoria2');
    if (n2)   n2.value   = user.nombre  || '';
    if (c2)   c2.value   = s.cuit       || '';
    if (cat2) cat2.value = s.categoria  || 'C';

    // ── Switches — cargar estado real ──
    const swFact  = document.getElementById('switchFactAuto2');
    const swEnvio = document.getElementById('switchEnvioAuto2');
    if (swFact)  swFact.checked  = s.factAuto  !== false; // default true
    if (swEnvio) swEnvio.checked = s.envioAuto !== false; // default true

  } catch (e) {
    console.warn('cargarConfigVista:', e.message);
  }
}

/**
 * Guarda un switch de configuración via PATCH /api/me/settings.
 * Usado por factAuto y envioAuto.
 */
async function guardarSwitch(key, value) {
  try {
    ...
    const statusId = key === 'factAuto' ? 'cfgAutoStatus2' : 'cfgAutoStatus2';
    ...
    toast(value
      ? `${key === 'factAuto' ? 'Facturación' : 'Envío'} automático activado`
      : `${key === 'factAuto' ? 'Facturación' : 'Envío'} automático desactivado`,
      'success'
    );
  } catch (e) {
...
function toggleClave() {
  ...
  const icon  = document.getElementById('cfgEyeIcon');
  if (!input) return;
...
async function verificarEstadoArca() {
  try {
    const { user } = await api.get('/api/me');
    actualizarEstadoARCA(user);
  } catch (e) {
    console.warn('verificarEstadoArca:', e.message);
  }
}

function actualizarEstadoARCA(user) {
  const s             = user?.settings || {};
  const statusBadge   = document.getElementById('arcaStatusBadge');
  const inputCuit     = document.getElementById('arcaCuit');
  const inputClave    = document.getElementById('arcaClave');
  const btnSync       = document.getElementById('btnArcaSync');
  const progressEl    = document.getElementById('arcaProgress');

  if (s.arcaStatus === 'vinculado') {
    if (statusBadge) {
      statusBadge.innerHTML = `
        <div class="status-dot" style="background:#10b981;box-shadow:0 0 6px #10b981"></div>
        <span style="color:#10b981">Vinculado ✓</span>`;
    }
    if (inputCuit)  { inputCuit.value  = s.cuit || ''; inputCuit.disabled  = true; }
    if (inputClave) { inputClave.placeholder = '••••••••'; inputClave.disabled = true; }
    if (btnSync) {
      btnSync.innerHTML  = '<span class="material-icons" style="font-size:16px!important">check_circle</span> Vinculado correctamente';
      btnSync.style.background = 'rgba(16,185,129,0.12)';
      btnSync.style.color      = '#10b981';
      btnSync.style.boxShadow  = '0 4px 16px rgba(16,185,129,.2)';
    ...
    if (inputClave) { inputClave.placeholder = '••••••••'; inputClave.disabled = true; }
    if (btnSync) {
      btnSync.innerHTML = '<span class="material-icons" style="font-size:16px!important">hourglass_top</span> Pendiente de revisión';
      btnSync.disabled  = true;
    }
  } else {
    // sin_vincular o error
    if (statusBadge) {
      statusBadge.innerHTML = `
        <div class="status-dot checking"></div>
        <span>Sin vincular</span>`;
    }
    if (inputCuit)  inputCuit.disabled  = false;
...
async function iniciarSyncArca() {
  ...
  if (!cuit || !clave) {
    toast('Ingresá CUIT y Clave Fiscal', 'warn');
    return;
  }
  ...
  try {
    ...
    // Animación de pasos (visual solamente — el proceso real lo gestiona el admin)
    ...
    const nextStep = () => {
      ...
      if (current >= steps.length) {
        const title = document.getElementById('arcaProgressTitle');
        if (title) title.textContent = '¡Solicitud enviada!';
        const spinner = document.getElementById('arcaSpinner');
        if (spinner) spinner.style.display = 'none';
        actualizarEstadoARCA({ settings: { arcaStatus: 'pendiente', cuit } });
        toast('✅ Datos enviados. Validando con ARCA (hasta 24hs)…', 'success');
        return;
      }
      const step = document.getElementById(steps[current]);
...
/* ── EMITIR CAE (CORREGIDO — llama a la API real) ───── */
async function emitir(orderId) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      setTimeout(() => _recargarDashConPeriodo(), 500);
    }
  } catch (e) {
...
function _parseFecha(str) {
  if (!str) return 0;
  const p = str.split('/');
  if (p.length === 3) return new Date(`20${p[2]}-${p[1]}-${p[0]}`).getTime() || new Date(`${p[2]}-${p[1]}-${p[0]}`).getTime();
  return 0;
}
...
function _iniciarPeriodo() {
  ...
  const d = document.getElementById('calDesde');
  const h = document.getElementById('calHasta');
...
async function cargarTodosComprobantes() {
  document.getElementById('manualesBody').innerHTML =
    `<tr><td colspan="8" style="text-align:center;padding:40px;color:var(--text-3);font-size:13px">Cargando…</td></tr>`;
  ...
  try {
    ...
    document.getElementById('manualesBody').innerHTML =
      `<tr><td colspan="8" style="text-align:center;padding:40px;color:var(--red);font-size:12px">Error: ${err.message}</td></tr>`;
  }
}
...
function limpiarBusqueda() {
  const el = document.getElementById('compBuscar');
  if (el) el.value = '';
  const clearBtn = document.getElementById('btnClearSearch');
...
function renderComprobantes(lista) {
  ...
  tbody.innerHTML = lista.map((c, i) => {
    ...
    const emitidoBtn = c.estado === 'emitido'
      ? `<button class="act-btn act-done" disabled title="CAE emitido ✓">
           <svg width="13" height="13" viewBox="0 0 14 14" fill="none">
             <path d="M2.5 7l3 3 6-6" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/>
           </svg>
         </button>`
      : `<button class="act-btn" title="Emitir CAE" onclick="emitir('${c._id || c.id}')">
           <svg width="13" height="13" viewBox="0 0 14 14" fill="none">
             <path d="M7 1.5l5.5 10H1.5L7 1.5z" stroke="currentColor" stroke-width="1.3" stroke-linejoin="round"/>
             <path d="M7 5.5v3" stroke="currentColor" stroke-width="1.3" stroke-linecap="round"/>
             <circle cx="7" cy="10" r=".6" fill="currentColor"/>
    ...
    <tr style="animation:rowIn .3s ease ${i*35}ms both">
      ...
      <td style="max-width:170px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;font-size:12px;color:var(--text-2)">${c.concepto || ''}</td>
      ...
      <td style="text-align:center">
        <div class="comp-actions" style="justify-content:center">
          ...
          ${emitidoBtn}
          <button class="act-btn" title="Enviar mail"  onclick="enviarMail('${c._id || c.id}')">
            <svg width="13" height="13" viewBox="0 0 14 14" fill="none">
              <rect x="1.5" y="3" width="11" height="8" rx="1.5" stroke="currentColor" stroke-width="1.3"/>
              <path d="M1.5 5l5.5 3.5L12.5 5" stroke="currentColor" stroke-width="1.2"/>
            </svg>
...
/* ── CALENDARIO / PERÍODO COMPROBANTES ─────────────── */
function toggleCalendario() {
  ...
  if (open) {
    setTimeout(() => document.addEventListener('click', function _cls(e) {
...
function aplicarRangoCustom() {
  ...
  if (d) {
    ...
    const label = _rangoHasta ? `${fmt(_rangoDesde)} → ${fmt(_rangoHasta)}` : `Desde ${fmt(_rangoDesde)}`;
    document.getElementById('btnPeriodoLabel').textContent = label;
  }
  filtrarComprobantes();
}
...
function abrirNuevaEmision() {
  ...
  ['emCliente','emEmail','emConcepto','emMonto'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.value = '';
  });
  const tipo = document.getElementById('emTipo');
  if (tipo) tipo.value = 'Factura C';
}
...
async function registrarEmision() {
  ...
  try {
    ...
    if (vistaActual === 'comprobantes') cargarTodosComprobantes();
    _recargarDashConPeriodo();
  } catch (e) {
...
async function cargarIntegraciones() {
  ...
  try {
    ...
    // Actualizar toggles en las cards
    const connected = {};
    (list || []).forEach(i => { connected[i.platform] = i; });
    ...
    ['woocommerce','mercadolibre','tiendanube','empretienda','rappi','vtex'].forEach(p => {
      ...
      if (desc) {
        desc.textContent = intg
          ? (intg.status === 'active' ? `✓ ${intg.storeName || intg.storeId}` : '⚠ Error de conexión')
          : 'Sin conectar';
        desc.style.color = (intg && intg.status === 'active') ? 'var(--green)' : '';
      }
    });
    ...
    if (!list || !list.length) {
      cont.innerHTML = `<div class="neg-empty">
        <span class="material-icons" style="font-size:32px;opacity:.3">store_off</span>
    ...
    cont.innerHTML = `<div style="color:var(--text-3);font-size:12px;padding:20px">Error cargando integraciones: ${e.message}</div>`;
  }
}
...
async function desconectar(id) {
  ...
  try {
    ...
    toast('Tienda desconectada', 'warn');
    cargarIntegraciones();
...
function toggleIntegracion(platform, enabled) {
  ...
  // En esta versión el toggle no activa/desactiva via API — solo visual
  // Para activar usa el botón Configurar
}
...
function abrirConexion(plataforma) {
  ...
  const NOMBRES = {
    ...
    tiendanube:'Tienda Nube', empretienda:'Empretienda', rappi:'Rappi', vtex:'VTEX',
  };
  document.getElementById('negModalTitle').innerHTML =
    `<span class="material-icons" style="color:var(--orange-2)">link</span> Conectar ${NOMBRES[plataforma] || plataforma}`;

  const forms = {
    woocommerce:'negFormWoo', mercadolibre:'negFormML',
    tiendanube:'negFormTN', empretienda:'negFormEM', rappi:'negFormRappi', vtex:'negFormVTEX',
  };
...
async function verificarSuscripcion() {
  try {
    const { user } = await api.get('/api/me');
    if (user?.plan === 'pro') {
      mostrarSuscripcionActiva(user.creadoEn);
    } else {
...
function mostrarSuscripcionActiva(fechaAlta) {
  ...
  document.getElementById('susc-cargando').style.display = 'none';
  if (fechaAlta) {
    document.getElementById('susc-fecha-alta').textContent =
      'Activa desde ' + new Date(fechaAlta).toLocaleDateString('es-AR');
  }
}
...
function conmutarEstado() {
  ...
  renderStatus(!on);
  toast(!on ? 'AFIP: modo activo' : 'AFIP: modo pausado', !on ? 'success' : 'warn');
}

function verPDF(id)       { toast('PDF no disponible en esta versión', 'info'); }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // Cargar usuario y dashboard inicial
  try {
    const { user } = await api.get('/api/me');

    // Actualizar estado ARCA en topbar
    ...
    // Guardar globalmente para otros módulos
  ...
  // Cargar stats del período actual
  await _recargarDashConPeriodo().catch(() => cargarDashboard(MOCK));

  // Verificar estado AFIP en background
  api.get('/api/afip/estado')
    .then(r => renderStatus(r.online))
    .catch(() => renderStatus(false));
});

// _recargarDashConPeriodo retorna Promise para poder awaitar en init
const _recargarDashConPeriodoProm = _recargarDashConPeriodo;
