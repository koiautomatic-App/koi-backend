...
async function getAfipToken(cuit) {
  ...
  // Construir TRA
  const now     = new Date();
  const genTime = new Date(now.getTime() - 60_000).toISOString().replace('Z', '-03:00');
  const expTime = new Date(now.getTime() + 12 * 3600_000).toISOString().replace('Z', '-03:00');
...
<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/"
  ...
  const ticket  = xmlDoc.getElementsByTagName('loginTicketResponse')[0];
  if (!ticket) throw new Error('WSAA: respuesta inválida — ' + res.data.slice(0, 200));

  const token  = ticket.getElementsByTagName('token')[0]?.textContent;
  const sign   = ticket.getElementsByTagName('sign')[0]?.textContent;
  const expStr = ticket.getElementsByTagName('expirationTime')[0]?.textContent;
  const expiry = expStr ? new Date(expStr).getTime() : Date.now() + 12 * 3600_000;

  if (!token || !sign) throw new Error('WSAA: token o sign ausente en respuesta');

  tokenCache.set(cuit, { token, sign, expiry });
  console.log(`✅ Token AFIP/WSAA obtenido para CUIT ${cuit}`);
  return { token, sign };
}
