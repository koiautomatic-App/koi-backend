...
const IntegrationSchema = new mongoose.Schema({
  ...
  platform: {
    type:     String,
    required: true,
    enum:     ['woocommerce', 'tiendanube', 'empretienda', 'mercadolibre', 'rappi', 'vtex'],
  },
  ...
  status: {
    type:    String,
    default: 'active',
    enum:    ['active', 'paused', 'error', 'revoked', 'pending'],
  },
...
// ── TOGGLE activa/desactiva integración ─────────────────────
app.post('/api/integrations/:platform/toggle', requireAuthAPI, async (req, res) => {
  try {
    ...
    res.json({ ok: true, status: integration.status });
  } catch(e) { res.status(500).json({ error: 'Error interno' }); }
});

// ── VTEX: guardar App Key + App Token ────────────────────────
app.post('/api/integrations/vtex', requireAuthAPI, async (req, res) => {
  try {
    const { accountName, appKey, appToken } = req.body;
    if (!accountName || !appKey || !appToken)
      return res.status(400).json({ error: 'Faltan accountName, appKey y appToken.' });

    // Verificar credenciales contra la API de VTEX
    const vtexRes = await axios.get(
      `https://${accountName}.vtexcommercestable.com.br/api/catalog_system/pvt/products/GetProductAndSkuIds`,
      {
    ...
    if (!vtexRes || vtexRes.status !== 200)
      return res.status(401).json({ error: 'Credenciales inválidas o sin acceso.' });

    const integration = await Integration.findOneAndUpdate(
      { userId: req.userId, platform: 'vtex', storeId: accountName },
      {
        userId:    req.userId,
        platform:  'vtex',
        storeId:   accountName,
        storeName: accountName,
        storeUrl:  `https://${accountName}.vtexcommercestable.com.br`,
        status:    'active',
        errorLog:  '',
        'credentials.consumerKey':    encrypt(appKey),
        'credentials.consumerSecret': encrypt(appToken),
      },
      { upsert: true, new: true, setDefaultsOnInsert: true }
    );

    console.log(`✅ VTEX integrado: ${accountName} → usuario ${req.userId}`);
    res.json({ ok: true, message: 'VTEX conectado correctamente.' });
  } catch(e) { console.error('VTEX connect:', e.message); res.status(500).json({ error: 'Error interno.' }); }
});
...
function normalizeOrder(platform, rawPayload) {
  switch(platform) {
    ...
    case 'vtex': {
      const clientData = rawPayload.clientProfileData || {};
      return {
        externalId:    rawPayload.orderId?.toString() || rawPayload.sequence?.toString(),
        customerName:  `${clientData.firstName || ''} ${clientData.lastName || ''}`.trim(),
        customerEmail: clientData.email || '',
        customerDoc:   (clientData.document || '').replace(/\D/g,'') || '0',
        amount:        parseFloat(rawPayload.value) / 100 || 0,  // VTEX usa centavos
        currency:      rawPayload.storePreferencesData?.currencyCode || 'BRL',
      };
    }
...
// ── WEBHOOK VTEX ─────────────────────────────────────────────
// VTEX Feed/Hook: configurar en Admin VTEX → Inventory & Shipping → Orders → Hook
app.post('/webhook/vtex/:secret', async (req, res) => {
  res.status(200).send('OK');
  const { secret } = req.params;
  // VTEX envía Domain + State, solo procesar órdenes en "payment-approved"
  if (req.body.State !== 'payment-approved') return;
  try {
    const integration = await Integration.findOne({ platform: 'vtex', webhookSecret: secret, status: 'active' });
    if (!integration) return;
    const appKey   = integration.getCredential('consumerKey');
    const appToken = integration.getCredential('consumerSecret');
    const orderId  = req.body.OrderId;
    const orderRes = await axios.get(
      `${integration.storeUrl}/api/oms/pvt/orders/${orderId}`,
      { headers: { 'X-VTEX-API-AppKey': appKey, 'X-VTEX-API-AppToken': appToken } }
    );
    const normalized = normalizeOrder('vtex', orderRes.data);
    if (!normalized) return;
    await processOrder(integration, normalized, orderRes.data);
  } catch(e) { console.error('❌ VTEX webhook error:', e.message); }
});
